...
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𝙎𝙊𝙐𝙍𝘾𝞝 𝙕𝞝 - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/UI_XB) 🗑♻️𓆪\n**𓍹━─━─━─━─𝙕𝞝𝘿─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @XSCIBOT ")
...
async def permalink(mention):
    ...
    if user.id == 5650717789 or user.id == 6348876717 or user.id == 5650717789:
...
async def permalink(mention):
    ...
    if user.id == 5650717789 or user.id == 6348876717 or user.id == 5650717789:
        return await edit_or_reply(mention, f"**╮ ❐ لك هـذا داد مودي . . شما حجيت ماكـدر اوصفـه 🙊💘 ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
